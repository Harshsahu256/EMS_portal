...
//         if (res?.success) {
//           const businessNews = res.data.filter(item => {
//             const categoryName = item.category?.name?.toLowerCase() || "";
//             return categoryName === "business" || categoryName === "बिज़नेस";
//           });
...
//       <h2 className="fw-bold border-bottom pb-2 mb-4">बिज़नेस</h2>
...
//                   {/* ✅ Summary below title - 2 line truncation */}
//                   <p
...
const BusinessListPage = () => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      setLoading(true); // Added setLoading(true) here for consistency
      try {
        const res = await allNews();
        if (res?.success) {
          const businessNews = res.data.filter(item => {
