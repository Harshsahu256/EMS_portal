  ...
  return (
    <Container className="my-4">
      ...
      {newsData.map(article => {
        const tags = [];
        if (article.category?.name && !["sports", "खेल"].includes(article.category.name.toLowerCase())) {
          tags.push(article.category.name);
        }
        if (article.subCategory?.name) tags.push(article.subCategory.name);

        // ✅ Fallback logic for summary (2 lines)
        const summaryText =
          article.summary_hi ||
          article.summary_en ||
          article.description ||
        ...
        return (
          <React.Fragment key={article._id || article.slug_en}>
            <Row className="mb-3 g-3">
              {/* Left Image */}
              <Col xs={12} md={4} lg={3}>
                <Link to={`/news/${article.slug_en}`} state={{ relatedArticles: newsData }} style={linkStyle}>
                  <Image
              ...
              <Col xs={12} md={8} lg={9} className="d-flex flex-column">
                <Link to={`/news/${article.slug_en}`} state={{ relatedArticles: newsData }} style={linkStyle} className="flex-grow-1">
                  {/* Title */}
                  <h5
