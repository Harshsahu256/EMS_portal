...
const ThoughtsListPage = () => {
  ...
  if (newsData.length === 0) return <Alert variant="warning">कोई Thoughts खबर उपलब्ध नहीं है।</Alert>;
  ...
  return (
    <Container className="my-4">
      ...
      {newsData.map(article => {
        const tags = [];
        if (article.category?.name && !["thoughts", "विचार"].includes(article.category.name.toLowerCase())) {
          ...
          <React.Fragment key={article._id || article.slug_en}>
            <Row className="mb-3 g-3">
              ...
              <Col xs={12} md={8} lg={9} className="d-flex flex-column">
                <Link to={`/news/${article.slug_en || article._id}`} state={{ relatedArticles: newsData }} style={linkStyle} className="flex-grow-1">
                  {/* Title */}
                  <h5
