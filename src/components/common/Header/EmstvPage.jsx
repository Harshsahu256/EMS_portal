...
import React, { useState, useEffect } from "react";
import { Container, Row, Col, Spinner, Image, Button, Dropdown } from "react-bootstrap";
// Updated icons imported: FaThumbsUp (for likes), FaCommentDots (for comments), FaShareAlt (for share)
  ...
  <div className="mb-3">
    ...
    <div
      ...
      style={{
        ...
        transition: "transform 0.2s ease-in-out",
        backgroundColor: "#000",
        height: "180px", // card height - kept inline as it's a fixed size
      }}
    >
      <Image
        src={thumbnail}
...
const EmstvPage = () => {
  ...
  // Share functionality
  const handleShare = async (title, videoUrl) => {
    // Construct the full URL for the video page (assuming your video player page path is `/video/:id`)
    // You might need to adjust this URL construction based on your actual routing.
    const fullShareUrl = `${window.location.origin}/video/${videoUrl}`; // Using videoUrl as id for simplicity, adjust if your video ID is different
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          url: fullShareUrl, // Share the actual video page URL
        });
      ...
      // You can also implement a custom share modal here if needed
    }
  };
      ...
      <div className="d-flex flex-wrap gap-2 mb-3">
        ...
        {hiddenCategories.length > 0 && (
          <Dropdown>
            ...
            <Dropdown.Menu className="rounded shadow-sm border-0 py-1"> {/* Bootstrap utility classes used for styling */}
              {hiddenCategories.map((cat) => (
                <Dropdown.Item
                  key={cat}
                  onClick={() => setSelectedCategory(cat)}
                  // Bootstrap's 'active' class with 'bg-danger', 'text-white', 'fw-bold' for active state, 'fw-bold' for others
                  className={selectedCategory === cat ? "active bg-danger text-white fw-bold" : "fw-bold"}
                >
                  {cat}
                </Dropdown.Item>
              ))}
      ...
      <Row className="g-3">
        {filteredVideos.map((video) => (
          <Col key={video._id} xs={12} sm={6} md={4} lg={3}>
            ...
            />
            {/* Updated icons (Likes, Comments, Share) below the video title */}
            <div className="d-flex justify-content-between w-100 mt-2"> {/* w-100 and justify-content-between for full width distribution */}
              <Button variant="link" className="text-dark px-2 py-1 text-decoration-none" title="Likes" onClick={() => { /* Implement likes functionality */ }}>
                <FaThumbsUp size={18} className="me-1" /> 0 {/* Example: Display 0 likes */}
              </Button>
              <Button variant="link" className="text-dark px-2 py-1 text-decoration-none" title="Comments" onClick={() => { /* Implement comments functionality */ }}>
                <FaCommentDots size={18} className="me-1" /> 0 {/* Example: Display 0 comments */}
              </Button>
              <Button
                variant="link"
                className="text-dark px-2 py-1 text-decoration-none"
                title="Share"
                onClick={() => handleShare(video.title, video._id)} // Passing video ID for share URL
              >
                <FaShareAlt size={18} className="me-1" /> Share
              </Button>
            </div>
          </Col>
        ))}
